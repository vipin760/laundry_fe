      ...
      body: Column(children: [
        ...
        Expanded(child: slotsAsync.when(
          ...
          data: (result) {
            ...
            final slots = (result?.pickupSlots ?? [])
                .where((s) => !s.isFallback)
                .toList();
            if (slots.isEmpty) {
              return SlotEmptyView('pickup', dateLabel: _label(_dateIndex));
            }
            final hasScheduled = slots.any((s) => !s.isInstant);
            return ListView(
              padding: const EdgeInsets.fromLTRB(16, 16, 16, 100),
              children: [
                const SlotSectionLabel('AVAILABLE PICKUP SLOTS'),
                ...
                if (!hasScheduled)
                  SlotEmptyView('pickup', dateLabel: _label(_dateIndex)),
              ],
            );
          },
        )),
